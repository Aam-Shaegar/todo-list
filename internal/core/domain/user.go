...
func NewUser(id, version int, phoneNumber *string, fullname string) User {
	return User{
		ID:          id,
		Version:     version,
		FullName:    fullname,
...
func (u *User) Validate() error {
	...
	if u.PhoneNumber != nil {
		...
		if !re.MatchString(*u.PhoneNumber) {
			return fmt.Errorf("invalid PhoneNumber format: %w", core_error.ErrInvlalidArgument)
		}
...
func (p *UserPatch) Validate() error {
	if p.FullName.Set && p.FullName.Value == nil {
		return fmt.Errorf("'FullName' can't be patched to NULL: %w", core_error.ErrInvlalidArgument)
	}
